...
def parse_bawag_csv(
    ...
    *,
    ...
    for line_number, row in enumerate(csv.reader(lines, delimiter=";"), start=1):
        ...
        if len(row) != 6:
            raise ValueError(f"BAWAG CSV row {line_number} has {len(row)} fields; expected 6")
        ...
        if currency.upper() != "EUR":
            raise ValueError(f"BAWAG CSV row {line_number} uses unsupported currency {currency!r}")

        merchant, city = parse_narrative(narrative)
        title = merchant or narrative.split("|", maxsplit=1)[0].strip()
        city = city if merchant else ""
        source_id = sha256("\x1f".join(row).encode("utf-8")).hexdigest()
